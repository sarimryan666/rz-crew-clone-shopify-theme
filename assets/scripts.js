...
class CartItems extends HTMLElement {
            ...
            .then(async (state) => {
                ...
                this.getSectionsToRender().forEach((section) => {
                    const elementToReplace =
                        (section.selector ? document.getElementById(section.id) ? .querySelector(section.selector) : null) ||
                        document.getElementById(section.id);

                    const sectionHtml = parsedState.sections[section.section];
                    const selector = section.selector || ".shopify-section";
                    const sectionElement = this.htmlParser.parseFromString(sectionHtml, "text/html").querySelector(selector);
                    if (sectionElement) {
                        elementToReplace.innerHTML = sectionElement.innerHTML;
                    }
                });
        ...
        .join(",")}&v=${Date.now()}`,
            ...
            .then((response) => {
                this.getSectionsToRender().forEach((section) => {
                    ...
                    const sectionHtml = response[section.section];
                    const selector = section.selector || ".shopify-section";
                    const sectionElement = this.htmlParser.parseFromString(sectionHtml, "text/html").querySelector(selector);
                    if (sectionElement) {
                        elementToReplace.innerHTML = sectionElement.innerHTML;
    ...
    getSectionInnerHTML(html, selector = ".shopify-section") {
        return (this.htmlParser || new DOMParser()).parseFromString(html, "text/html").querySelector(selector) ? .innerHTML || "";
    }
...
class MiniCart extends CartItems {
    ...
    open(opener) {
        ...
        this.dispatchEvent(new CustomEvent("mini-cart:opened", {
            bubbles: true
        }));
    }
    ...
    close() {
        this.toggle.close();
        this.dispatchEvent(new CustomEvent("mini-cart:closed", {
            bubbles: true
        }));
    }
...
// EASYSEARCH CUSTOM START
document.addEventListener('easysearch_garage_rendered', function() {
    let garageLogo = document.querySelector(".easysearch-garage-logo-holder");
    let selectedItem = document.querySelector(".easysearch-garage-widget .easysearch-garage-item[data-selected='1']");
    if (garageLogo && selectedItem) {
        garageLogo.classList.add("easy-selected");
    }
});
// EASYSEARCH CUSTOM FINISH